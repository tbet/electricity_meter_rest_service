...
public class SmlFileWorker
{
	...
	public JsonValues getSMLAsJson()
	{
		...
		values.setAppliedMethod("I-INFO");
		values.setTimestamp(instant);

		if(energy_180 > -1)
		{
			values.setV_1_8_0((((float)energy_180)/10000));
		}
		if(energy_181 > -1)
		{
			values.setV_1_8_1((((float)energy_181)/10000));
		}
		if(energy_182 > -1)
		{
			values.setV_1_8_2((((float)energy_182)/10000));
		}
		if(energy_280 > -1)
		{
			values.setV_2_8_0((((float)energy_280)/10000));
		}
		if(power > -1)
		{
			values.setPower((float)power);
		}
		values.setServerID(server_id.toHexString());
		values.setSecindex(((Unsigned32)actSensorTime.getChoice()).getVal());
		return values;
	}
